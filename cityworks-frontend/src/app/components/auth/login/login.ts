...
export class Login {
  ...
  // Forgot password (step 1: get token, step 2: reset)
  ...
  submitForgotUsername() {
    ...
    this.http.post<any>(`${BASE}/forgot-username`, { email: this.forgotUsernameEmail }).subscribe({
      ...
      error: (err) => {
        this.forgotUsernameMsg = err?.error?.message || 'Email not found.';
        this.forgotUsernameLoading = false;
      },
    });
  }
  ...
  submitForgotPassword() {
    ...
    this.http
      ...
      .subscribe({
        ...
        error: (err) => {
          this.forgotPasswordMsg = err?.error?.message || 'Username not found.';
          this.forgotPasswordLoading = false;
        },
      });
  }
  ...
  submitResetPassword() {
    ...
    this.http
      ...
      .subscribe({
        ...
        error: (err) => {
          this.resetMsg = err?.error?.message || 'Reset failed.';
          this.resetLoading = false;
        },
      });
  }
