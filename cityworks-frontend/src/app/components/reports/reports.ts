...
export class Reports implements OnInit {
  ...
  openGenerateModal() {
    this.reportForm = { scope: 'REQUESTS', parametersJSON: '{}' };
  ...
  generateReport() {
    this.saving = true;
    this.http
      .post<any>(`${BASE}/reports`, {
        scope: this.reportForm.scope,
        parametersJSON: this.reportForm.parametersJSON,
        generatedBy: this.auth.getUserId(),
      })
      .subscribe({
        next: () => {
          this.saving = false;
  ...
  openPackageModal() {
    this.packageForm = { periodStart: '', periodEnd: '' };
    this.showPackageModal = true;
  }

  generatePackage() {
    if (!this.packageForm.periodStart || !this.packageForm.periodEnd) {
      this.toast.warning('Please select both period start and end dates.');
      return;
    }
    this.saving = true;
    this.http.post<any>(`${BASE}/audit-packages`, this.packageForm).subscribe({
      next: () => {
        ...
        this.loadPackages();
        this.toast.success('Audit package created successfully.');
      },
      error: (err) => {
        this.saving = false;
        this.toast.error(extractError(err));
